        ...
        <el-card class="box-card">
            ...
            <el-button v-waves class="filter-item" type="primary" icon="el-icon-search" @click="handleFilter">
                ...
                <el-button v-permission="['admin']" class="filter-item" style="margin-left: 10px;" type="primary"
                    icon="el-icon-edit" @click="handleCreate" size="small">
                    Add
                </el-button>
...
<script>
import { factorList, factorInfo, factorAdd, factorEdit, factorDelete } from '@/api/factor'
...
export default {
    ...
    methods: {
        getList() {
            ...
            factorList(this.listQuery).then(response => {
        ...
        handleUpdate(row) {
            let tasks = []
            tasks.push(factorInfo({ 'id': row.id }))
            Promise.all(tasks).then(
                (results) => {
                    console.log('userInfo', results[0])
                    this.temp = results[0].result
                    this.dialogFormVisible = true
                }
            )
